...
import type { UmlautWord } from '../types';
import { SpeakerIcon, MicrophoneIcon, StopIcon } from './IconComponents';
import { useSpeech } from '../contexts/SpeechContext';

interface UmlautWordDisplayProps {
  data: UmlautWord;
}

const UmlautWordDisplay: React.FC<UmlautWordDisplayProps> = ({ data }) => {
  ...
  return (
    <div className={`bg-slate-50 rounded-lg border border-slate-200 p-4 w-full md:w-56 text-center shadow-sm transition-all ${isThisCardSpeaking ? 'ring-2 ring-[var(--theme-color)]' : ''}`}>
...
export default UmlautWordDisplay;
